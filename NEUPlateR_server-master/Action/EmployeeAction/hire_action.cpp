#include "hire_action.h"
#include "Business/Employee/employee_controller.h"
#include <QJsonArray>

IMPLEMENT_ACTION(hire_employees, CHireAction)

void CHireAction::run()
{
    QJsonArray username;
    try{
    ...
    for(const auto &u : username){
        code = Employee::remove_state(u.toString(), EmployeeState::RESIGNATION);
        if(StatusCode::EMPTY_QUERY == code){
            ...
            resp->set_desc("hire employee error");
            return;
        }
    }

    resp->set_status_code(StatusCode::SUCCESS);
    resp->set_desc("hire employees successful");
}
